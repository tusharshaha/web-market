    ...
    <div className="space-y-4">
        ...
        <ul className="ml-3 mt-3 space-y-2">
          {categories.map((ele, i) => (
            <li key={i} className="flex items-center justify-between">
              <label
                htmlFor={ele.title}
                className="flex items-center gap-2 cursor-pointer hover:text-primary w-full"
              >
      ...
      <div className="bg-slate-50 p-3 shadow-md">
        ...
        <div className="mt-3 flex items-center flex-wrap gap-2">
          {skills.map((ele, i) => (
            <button
              key={i}
              className="btn btn-xs capitalize tracking-wider btn-outline btn-primary"
            >
              {ele}
            </button>
          ))}
          <button className="btn btn-xs text-white capitalize btn-primary">
            Add New +
          </button>
        </div>
      </div>
      ...
      <div className="bg-slate-50 p-3 shadow-md">
        ...
        <div className="flex items-center justify-between">
          <button className="btn btn-primary btn-xs tracking-widest text-white">
            Filter
          </button>
          <p className="">
            {minPrice} - {maxPrice} Years
          </p>
        </div>
      </div>
    </div>
